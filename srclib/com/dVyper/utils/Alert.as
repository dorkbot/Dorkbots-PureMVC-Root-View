package com.dVyper.utils {
	...
	public class Alert {
		...
		private static var alertOptionsStyles:Object = null;
		//
		...
		*/
		public static function init(stageReference:Stage, aAlertOptions:Object = null):void {
			stage = stageReference;
			alertOptionsStyles = aAlertOptions;
		}
		public static function show(Text:* = "Made by dVyper", ALERTOPTIONS:Object = null):void {
			...
			{
				alertOptions = new AlertOptions(ALERTOPTIONS, Text);
			}
			else
			{
				alertOptions = new AlertOptions(alertOptionsStyles, Text);
			}
		...
		private static function createButton(buttonText:String, alertOptions:AlertOptions):SimpleButton {
			...
			btnOverState.graphics.lineStyle(3, brightenColour(alertOptions.colour, -50));
			...
			btnUpState.graphics.lineStyle(1, brightenColour(alertOptions.colour, -50));
		...
		private static function createPrompt(alertOptions:AlertOptions):Sprite {
			...
			var myHeight:int = textField.height + 100;
			//var myHeight:int = textField.height + 65;
			//var myHeight:int = stage.stageHeight-40;
			...
			if (myWidth > stage.stageWidth) 
			{
				myWidth = stage.stageWidth - 20;
				textField.autoSize = TextFieldAutoSize.CENTER;
				textField.wordWrap = true;
				textField.width = stage.stageWidth-40;
			}
			else if (myHeight > stage.stageHeight) 
			{
			...
			{
				alertButtons[i].y = actualPrompt.height - 25;
		...
		private static function createTextField(alertOptions:AlertOptions):TextField {
			...
			var myTextField:TextField = new TextField();
			myTextField.textColor = alertOptions.textColour;
			if (alertOptions.widthOfText > 0) 
			{
				myTextField.wordWrap = true;
				myTextField.width = alertOptions.widthOfText;
			}
			...
			myTextField.htmlText = '<font face="Verdana" align="center">'+Text+'</font>';
			myTextField.x = (btnWidth/2)-(myTextField.width/2);
			return myTextField;
		}
		//
...
internal class AlertOptions {
	...
	public function AlertOptions(alertOptions:Object, Text:*):void {
		if (alertOptions != null) {
			...
			if (alertOptions.widthOfText != null) 
			{
				widthOfText = alertOptions.widthOfText;
			} 
			else 
			{
				widthOfText = 0;
